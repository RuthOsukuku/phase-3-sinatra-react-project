...
patients_data = [
  ...
  doctors = doctors_data.map do |doctor_data|
    Doctor.create(doctor_data)
  end
  
  appointments_data.each_with_index do |appointment_data, index|
    appointment_data[:patient_id] = patients[index % patients.size].id
    appointment_data[:doctor_id] = doctors[index % doctors.size].id
